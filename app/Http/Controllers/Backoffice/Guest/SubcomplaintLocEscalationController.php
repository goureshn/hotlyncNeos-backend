...
use App\Models\Service\ComplaintSublistLocEscalation;
...
class SubcomplaintLocEscalationController extends UploadController
{
    ...
    public function index(Request $request)
    {
		...
		return Datatables::of($datalist)
				->addColumn('levels', function ($data) {
					...
					$list = DB::table('services_complaint_sublist_loc_escalation')
	...
	public function selectItem(Request $request)
    {
		...
		$list = DB::table('services_complaint_sublist_loc_escalation as cse')			
			->where('cse.type_id', $type_id)
	...
	public function updateEscalationInfo(Request $request)
	{
		...
		$model = ComplaintSublistLocEscalation::find($id);

		if( empty($model) )
		{
			$model = new ComplaintSublistLocEscalation();
	...
	public function deleteEscalationInfo(Request $request)
	{
		...
		if( !empty($model) )
		{
			...
			$sql = sprintf('UPDATE `services_complaint_sublist_loc_escalation` SET `level` = `level` - 1 WHERE `type_id` = %d AND `severity_id` = %d AND `level` > %d',
								$type_id, $severity_id, $level);
