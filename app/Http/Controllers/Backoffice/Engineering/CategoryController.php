...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $partgroup = DB::table('eng_request_category as erc')
            ->where('erc.name', 'like', '%name%')
            ->select('erc.id', 'erc.name')
            ->pluck('name', 'id');

        $step = '0';

        return view('backoffice.wizard.engineering.category', compact('name', 'description', 'code'));
    }

    public function getGridData()
    {
        $datalist = DB::table('eng_request_category as erc')
            ->leftJoin('common_property as cp', 'erc.property_id', '=', 'cp.id')
            ->select(['erc.*','cp.name as cpname']);
