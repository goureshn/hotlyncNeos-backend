...
class SupplierController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $partgroup = DB::table('eng_supplier as es')
            ->where('es.supplier', 'like', '%name%')
            ->select('es.id', 'es.supplier')
            ->pluck('name', 'id');

        $step = '0';

        return view('backoffice.wizard.engineering.supplier', compact('name', 'description', 'code'));
    }

    public function getGridData()
    {
        $datalist = DB::table('eng_supplier as es')
            ->select(['es.*']);
