...
//--    var template = '<li id="contact_[ID]"><a href="javascript:void(0)" data-transition="slide">[NAME]</a></li>';
    ...
    var template = '<li id="contact_[ID]"><a data-transition="slide" class="mlistitem">[NAME]<span style="float:right">[DEPARTMENT]</span><span style="display:block">[PERIOD]<span style="float:right">[VIP]</span></span></a></li>';
    var listview = '';
    ...
    for (var i = 0; i < ctlistLocal.length; i++)
    {
        ...
        var dept_name = item[100];
        lisitem = lisitem.replace('[DEPARTMENT]', dept_name);

        if( item[99] == 2 )
            lisitem = lisitem.replace('[PERIOD]', item[101]);
        else    
            lisitem = lisitem.replace('<span style="display:block">[PERIOD]<span style="float:right">[VIP]</span></span>', '');

        if( item[102] == 'VIP' )
            lisitem = lisitem.replace('[VIP]', item[102]);
        else    
            lisitem = lisitem.replace('<span style="float:right">[VIP]</span>', '');
