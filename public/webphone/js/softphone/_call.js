...
//--        webphone_api.common.PrintCallSessions();
    ...
    webphone_api.$( "#page_call" ).keydown(function(event)
    {
        try{
            ...
            if (charCode === 13 || (event.altKey && event.which == 65)) // enter
            {
            ...
            else if ((charCode === 27 || charCode === 8) || (event.altKey && event.which == 88)) // ESC or Backspace
            {
                var active_popups = webphone_api.$.mobile.activePage.find(".messagePopup");
                if (!webphone_api.common.isNull(active_popups) && active_popups.length > 0)
                {
...
// attach click listeners   conference,transfer,numpad,mute,hold,speaker
    ...
    webphone_api.$('#btn_conference').on('click', function(event) { 
    ...
    webphone_api.$('#btn_conference').attr('title', webphone_api.stringres.get('hint_new_call'));
...
function holdIncomingCall(hstate){
    var request = {};
    request.current_url = window.location.host; //.origin if https
    request.hstate = hstate;
    request.currcallnumber = webphone_api.global.currcallnumber;

    postAjax('/frontend/call/holdIncomingCall', request, function(data){ console.log(data); });
...
return {
    ...
    CallfunctionsOnclick: CallfunctionsOnclick,
