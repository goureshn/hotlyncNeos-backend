...
(function(window, document, undefined) {
  ...
  angular.module('mgcrea.ngStrap.typeahead', [ 'mgcrea.ngStrap.tooltip', 'mgcrea.ngStrap.helpers.parseOptions' ]).provider('$typeahead', function() {
    ...
    this.$get = [ '$window', '$rootScope', '$straptooltip', '$$rAF', '$timeout', function($window, $rootScope, $straptooltip, $$rAF, $timeout) {
  ...
  angular.module('mgcrea.ngStrap.tooltip', [ 'mgcrea.ngStrap.core', 'mgcrea.ngStrap.helpers.dimensions' ]).provider('$straptooltip', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
      customClass: '',
      prefixClass: 'tooltip',
    ...
    this.$get = [ '$window', '$rootScope', '$bsCompiler', '$q', '$templateCache', '$http', '$animate', '$sce', 'dimensions', '$$rAF', '$timeout', function($window, $rootScope, $bsCompiler, $q, $templateCache, $http, $animate, $sce, dimensions, $$rAF, $timeout) {
      ...
      function TooltipFactory(element, config) {
        ...
        var promise = $straptooltip.$promise = $bsCompiler.compile(options);
        ...
        $straptooltip.$id = options.id || element.attr('id') || '';
        if (options.title) {
          scope.title = $sce.trustAsHtml(options.title);
        }
        scope.$setEnabled = function(isEnabled) {
          scope.$$postDigest(function() {
            $straptooltip.setEnabled(isEnabled);
          });
        };
        scope.$hide = function() {
          scope.$$postDigest(function() {
        ...
        $straptooltip.enter = function() {
          ...
          timeout = setTimeout(function() {
            if (hoverState === 'in') $straptooltip.show();
          }, options.delay.show);
        };
        $straptooltip.show = function() {
          if (!options.bsEnabled || $straptooltip.$isShown) return;
          scope.$emit(options.prefixEvent + '.show.before', $straptooltip);
          if (angular.isDefined(options.onBeforeShow) && angular.isFunction(options.onBeforeShow)) {
            options.onBeforeShow($straptooltip);
          }
          var parent;
          var after;
          ...
          tipScope = $straptooltip.$scope.$new();
          tipElement = $straptooltip.$element = compileData.link(tipScope, function(clonedElement, scope) {});
          tipElement.css({
            top: '-9999px',
            left: '-9999px',
            right: 'auto',
        ...
        $straptooltip.toggle = function(evt) {
          if (evt) {
            evt.preventDefault();
          }
        ...
        $straptooltip.setEnabled = function(isEnabled) {
          options.bsEnabled = isEnabled;
        };
        $straptooltip.setViewport = function(viewport) {
        ...
        $straptooltip.$applyPlacement = function() {
          ...
          $straptooltip.$viewport = options.viewport && findElement(options.viewport.selector || options.viewport);
          if (autoPlace) {
            var originalPlacement = placement;
            var viewportPosition = getPosition($straptooltip.$viewport);
        ...
        $straptooltip.$onKeyUp = function(evt) {
          if (evt.which === 27 && $straptooltip.$isShown) {
            $straptooltip.hide();
            evt.stopPropagation();
          }
        };
        ...
        function bindTriggerEvents() {
          ...
          angular.forEach(triggers, function(trigger) {
            if (trigger === 'click' || trigger === 'contextmenu') {
              element.on(trigger, $straptooltip.toggle);
              ...
              if (nodeName === 'button' && trigger !== 'hover') {
                element.on(isTouch ? 'touchstart' : 'mousedown', $straptooltip.$onFocusElementMouseDown);
              }
            }
          });
        }
        function unbindTriggerEvents() {
          ...
          for (var i = triggers.length; i--; ) {
            ...
            if (trigger === 'click' || trigger === 'contextmenu') {
              element.off(trigger, $straptooltip.toggle);
            } else if (trigger !== 'manual') {
              ...
              if (nodeName === 'button' && trigger !== 'hover') {
                element.off(isTouch ? 'touchstart' : 'mousedown', $straptooltip.$onFocusElementMouseDown);
              }
            }
          }
        }
        ...
        function unbindKeyboardEvents() {
          if (options.trigger !== 'focus') {
            ...
            element.off('keyup', $straptooltip.$onFocusKeyUp);
          }
        }
        ...
        function bindAutoCloseEvents() {
          $timeout(function() {
            ...
            $body.on('click', $straptooltip.hide);
        ...
        function unbindAutoCloseEvents() {
          if (_autoCloseEventsBinded) {
            ...
            $body.off('click', $straptooltip.hide);
        ...
        function getViewportAdjustedDelta(placement, position, actualWidth, actualHeight) {
          ...
          if (!$straptooltip.$viewport) return delta;
          var viewportPadding = options.viewport && options.viewport.padding || 0;
          var viewportDimensions = getPosition($straptooltip.$viewport);
        ...
        function destroyTipElement() {
          clearTimeout(timeout);
          if ($straptooltip.$isShown && tipElement !== null) {
            if (options.autoClose) {
              unbindAutoCloseEvents();
            }
            if (options.keyboard) {
          ...
          if (tipElement) {
            tipElement.remove();
            tipElement = $straptooltip.$element = null;
          }
        }
        return $straptooltip;
      }
  ...
  }).directive('bsTooltip', [ '$window', '$location', '$sce', '$parse', '$straptooltip', '$$rAF', function($window, $location, $sce, $parse, $straptooltip, $$rAF) {
    return {
      restrict: 'EAC',
      scope: true,
      link: function postLink(scope, element, attr, transclusion) {
        ...
        tooltip = $straptooltip(element, options);
        scope.$on('$destroy', function() {
  ...
  angular.module('mgcrea.ngStrap.timepicker', [ 'mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.helpers.dateFormatter', 'mgcrea.ngStrap.tooltip' ]).provider('$timepicker', function() {
    ...
    this.$get = [ '$window', '$document', '$rootScope', '$sce', '$dateFormatter', '$straptooltip', '$timeout', function($window, $document, $rootScope, $sce, $dateFormatter, $straptooltip, $timeout) {
      var isNative = /(ip[ao]d|iphone|android)/gi.test($window.navigator.userAgent);
      var isTouch = 'createTouch' in $window.document && isNative;
      if (!defaults.lang) {
  ...
  angular.module('mgcrea.ngStrap.select', [ 'mgcrea.ngStrap.tooltip', 'mgcrea.ngStrap.helpers.parseOptions' ]).provider('$select', function() {
    ...
    this.$get = [ '$window', '$document', '$rootScope', '$straptooltip', '$timeout', function($window, $document, $rootScope, $straptooltip, $timeout) {
      var isNative = /(ip[ao]d|iphone|android)/gi.test($window.navigator.userAgent);
      var isTouch = 'createTouch' in $window.document && isNative;
  ...
  angular.module('mgcrea.ngStrap.popover', [ 'mgcrea.ngStrap.tooltip' ]).provider('$popover', function() {
    ...
    this.$get = [ '$straptooltip', function($straptooltip) {
  ...
  angular.module('mgcrea.ngStrap.dropdown', [ 'mgcrea.ngStrap.tooltip' ]).provider('$dropdown', function() {
    ...
    this.$get = [ '$window', '$rootScope', '$straptooltip', '$timeout', function($window, $rootScope, $straptooltip, $timeout) {
      var bodyEl = angular.element($window.document.body);
  ...
  angular.module('mgcrea.ngStrap.datepicker', [ 'mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.helpers.dateFormatter', 'mgcrea.ngStrap.tooltip' ]).provider('$datepicker', function() {
    ...
    this.$get = [ '$window', '$document', '$rootScope', '$sce', '$dateFormatter', 'datepickerViews', '$straptooltip', '$timeout', function($window, $document, $rootScope, $sce, $dateFormatter, datepickerViews, $straptooltip, $timeout) {
      var isNative = /(ip[ao]d|iphone|android)/gi.test($window.navigator.userAgent);
      var isTouch = 'createTouch' in $window.document && isNative;
