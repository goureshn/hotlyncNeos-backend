app.service('GuestService', function ($rootScope, $http, AuthService) {
    ...
    this.getTaskInfo = function (task_id, lg_id, l_id) {
        var promiss = $http.get('/guest/guestservice/taskinfo?task_id=' + task_id + '&location_group=' + lg_id + '&location_id=' + l_id);
    ...
    {        
        var status_name = ['Completed', 'Open', 'Escalated','Timeout', 'Canceled', 'Scheduled', 'Unassigned' ];
    ...
    this.getStatusCssInEdit = function(row)
    {
        ...
        if( row.status_id == 9 )    // Closed
            return 'bg-status-t-unattended';

        if( row.status_id == 10 )    // Unassigned
            return 'bg-status-t-unattended';    
    }
