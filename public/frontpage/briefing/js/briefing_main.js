        ...
        function(  $rootScope, $scope,   $localStorage, $window, $state, $http, $interval, $timeout, AuthService, Base64, socket, liveserver, webNotification) {
            ...
            function onLoginSuccess(data) {
                ...
                $http({
                    ...
                    .then(function(response) {
                        if( response.data.code != 200 && response.data.code != 201  ) // no briefing
                        {
                            $scope.logout();
                            return;
                        }
            ...
            socket.on('server_live', function (message) {
                if( AuthService.isAuthenticated() == false ||  !AuthService.isValidModule('app.complaint.briefing_view')  )
                {
                    return;
                }
