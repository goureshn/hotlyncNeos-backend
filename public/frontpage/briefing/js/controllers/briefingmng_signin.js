...
app.controller('SigninFormController', function($scope, $rootScope, $http, $state, AuthService) {
    ...
    // already login
    if( AuthService.isAuthenticated() && AuthService.isValidModule('app.complaint.briefing_mng'))
    {
        $state.go('app.complaint.briefing_mng');
    }
    else
          ...
          $http.post('/auth/login', request)
              .then(function (response) {
                  if (response.data.code == '200') {
                    ...
                    var home_page = 'app.complaint.briefing_mng';
