...
app.controller('SigninFormController', function($scope, $rootScope, $http, $state, AuthService) {
    ...
    // already login
    if( AuthService.isAuthenticated() && AuthService.isValidModule('app.guestservice.guestinfo'))
    {
        $state.go('app.guestservice.guestinfo');
    }
    else
          ...
          $http.post('/auth/login', request)
              .then(function (response) {
	              if (response.data.code == '200') {
                    ...
                    var home_page = 'app.guestservice.guestinfo';
