app.controller('MycallMobileClassificationController', function ($scope, $rootScope, $http, $aside, $timeout, $uibModal, $window, $interval, toaster, AuthService) {
    ...
    $scope.groupby_num_text = "Group By Number";
    $scope.groupby_num_flag = 0;
   ...
   // getExtensionList();
    ...
    $scope.getDataList = function(tableState) {
        ...
        request.groupby = $scope.filter.groupby ? ($scope.filter.groupby):'';
        request.start_time = $scope.start_time;
        request.end_time = $scope.end_time;
        request.searchoption = search_option;
        var profile = AuthService.GetCredentials();
        request.property_id = profile.property_id;
        request.all_flag = ($scope.filter.all_flag) ? $scope.filter.all_flag : false ;
        request.call_type = [];
        ...
        $http({
            method: 'POST',
            url: '/frontend/callaccount/mymobilecall',
            data: request,
            headers: {'Content-Type': 'application/json; charset=utf-8'}
        })
        ...
        $scope.checkGroupBy =function() {
            if ($scope.groupby_num_text=='Ungroup Calls')
            {
                $scope.isLoading = true;
            ...
            {
                $scope.isLoading = true;
                $scope.filter.groupby = 'call_to';
               // $scope.getDataList();
                $scope.groupby_num_flag = 1;
                $scope.groupby_num_text = "Ungroup Calls";
        ...
        $scope.getViewDetail = function getViewDetail(row, index) {
            ...
            {
                if (row.call_to != $scope.datalist[i].call_to)
                    $scope.datalist[i].collapse = false;
            }
           // window.alert(row.collapse);
            if((!row.collapse || row.collapse==false))
            {
                // $http({
                //     method: 'POST',
                //     url: '/frontend/callaccount/mobiledetailcall',
                //     data: request,
                //     headers: { 'Content-Type': 'application/json; charset=utf-8' }
                // })
                //     .then(function (response) {
                        ...
                        if (row.selected == true)
                        {
                            ...
                            for (j = 0; j < row.details.length; j++) {
                                if (row.call_to == row.details[j].call_to) {
                                    row.details[j].selected = true;
                                    $scope.selected_sub.push(row.details[j]);
                                }
                            }
                        }
               // console.log(row);
                        // else
                        // {
                        //     $scope.detaillist.forEach(element => {

                        //         element.selected = false;
                        //         window.alert($scope.selected_sub.indexOf(element), 1);
                        //     });
    ...
    $scope.onChangeSelectedSingle = function (row, $index, main) {
//window.alert("HEre");
        if(row.selected==true)
            $scope.selected_sub.push(row);
        else
            $scope.selected_sub.splice($scope.selected_sub.indexOf(row));
        
        // if()    
         
        // window.alert("ARR:" + JSON.stringify($scope.selected_sub));
    }
    $scope.onChangeSelected = function(row,$index) {
        ...
        //window.alert("Here" + $scope.selected_count);
        if($scope.groupby_num_flag==1 && row.selected==true)
        {
            $scope.getViewDetail(row,$index);
        }
        else if ($scope.groupby_num_flag == 1 && row.selected == false)
        {
            var temp=[];
            
                  temp = $scope.selected_sub.filter(function (item) {
                     if (item.call_to == row.call_to)
                //         return item;
                //     else
                     item.selected=false;
          ...
          //  $scope.selected_sub  
            ...
            for (i = 0; i < temp.length;i++)
                {   
                  // window.alert("TEMP:" +temp[i].call_to);
            ...
            $scope.selected_sub=temp;
           // window.alert("ARR:" + JSON.stringify($scope.selected_sub));
               // window.alert("TEMP:"+JSON.stringify(temp));
    ...
    $scope.onChangeClassify = function(row) {
        if( row.classify_temp == 'Business' )
        {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: 'ClassifyPersonalController',
                size: size,
                resolve: {
                    call: function () {
                        return row;
    ...
    {
            selected_arr = $scope.selected_sub;
    }
    ...
    $scope.sendApproval = function() {
        ...
        $http({
            method: 'POST',
            url: '/frontend/callaccount/submitmobileapproval',
            data: request,
            headers: {'Content-Type': 'application/json; charset=utf-8'}
        })
            .then(function(response) {
    ...
    function submitComment(call, comment) {
        ...
        $http({
            method: 'POST',
            url: '/frontend/callaccount/submitcommentmobile',
            data: request,
            headers: {'Content-Type': 'application/json; charset=utf-8'}
        }).then(function(response) {
            $scope.getDataList();
...
app.controller('ClassifyPersonalController', function ($scope, $uibModalInstance, $http, AuthService, toaster, call) {
   // $scope.call = call;
    var temp,flag=0;
  for(i=0;i<call.length;i++)
  {
    ...
    if ($scope.call)   
    {
        $scope.call.contact_no = $scope.call.call_to;
...
app.controller('PhonebookCtrl', function ($scope, $rootScope, $window, call, $uibModalInstance, AuthService, $http, toaster, $interval) {
