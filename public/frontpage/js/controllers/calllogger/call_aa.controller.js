app.controller('AAController', function ($scope, $rootScope, $http, $httpParamSerializer, $timeout, $uibModal, $window, AuthService, toaster) {
    var MESSAGE_TITLE = 'Auto Attendant';
    ...
    $scope.ticket_height = $window.innerHeight - 88;
    ...
    $http.get('/frontend/call/autotypelist?property_id=' + profile.property_id)
        .then(function(response) {
    ...
    $http.get('/frontend/call/autocalltypelist?property_id=' + profile.property_id)
        .then(function(response) {
            $scope.calltypelist = response.data;
            for( var i = 0; i < $scope.calltypelist.length; i++) {
                $scope.calltypelist[i].id = i+1;
            }
            $scope.calltypelist.unshift(all);

            console.log()

            $scope.calltype_filter = {};
            for( var i = 0; i < $scope.calltypelist.length; i++)
                $scope.calltype_filter[$scope.calltypelist[i].id] = true;
        });
...
*/
    ...
    $scope.onChangeCallTypelist = function(call_type) {
        if( call_type.label == 'All' ) {
            for( var i = 0; i < $scope.calltypelist.length; i++) {
                $scope.calltype_filter[$scope.calltypelist[i].id] = $scope.calltype_filter[0];
            }
        }
        filterlist = [];
        var j = 0;
        for( var i = 0; i < $scope.calltypelist.length; i++) {
           if($scope.calltype_filter[$scope.calltypelist[i].id] == true) {
               if($scope.calltypelist[i].label !='All'||$scope.calltypelist[i].label != null){
                   filterlist[j] = $scope.calltypelist[i].label;
                   j++;
               }
           }
        }
        $scope.filters.filtername = 'call_type';
        $scope.filters.filtervalue = filterlist ;
  //      window.alert($scope.filters.filtervalue);
        $scope.getDataList();
    }
