app.controller('CampaignCreateController', function ($scope, $rootScope, $http, $interval, $timeout, $httpParamSerializer, AuthService, toaster, Upload) {
    ...
    function initCampaign() {
        $scope.campaign= {};
        $scope.campaign.property_id = profile.property_id;
        $scope.campaign.active = true;
        $scope.campaign.name = '';
        $scope.campaign.type = $scope.type_list[0];
        $scope.start_date = moment().format('YYYY-MM-DD');
        $scope.end_date = moment().format('YYYY-MM-DD');        

        $scope.onChangeType();

        $scope.campaign.reject_flag = true;
        $scope.campaign.sms_flag = true;
        $scope.campaign.email_flag = true;        
        $scope.campaign.sms_content = '';
        $scope.campaign.email_content = '';
        $scope.campaign.before_date = 0;
        $scope.campaign.every_date = 3;
        $scope.campaign.email_subject = '';
        $scope.campaign.trigger_at = '01:00';
        $scope.campaign.holiday = moment().format('YYYY-MM-DD');

        $scope.campaign.book_tags = [];
        $scope.campaign.guest_tags = [];
