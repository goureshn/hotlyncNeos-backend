define(['app', 'datatables.net', 'datatables.net-bs', 'directives/directive'], 
		function (app) {
	app.controller('ChannelCtrl', function ($scope, $compile, $timeout, $http, $window, $interval /*$location, $http, initScript */) {
		...
		$scope.menus = [
					...
					{link: '/callcenter/channel', name: 'Channel'},
				];
		...
		//end///
		$scope.fields = ['ID', 'Label'];
		function initDomData() {
			var $grid = $('#table_grid').dataTable( {
				processing: true,
				serverSide: true,
				order: [[ 0, "asc" ]], //column indexes is zero based
				ajax: '/backoffice/callcenter/wizard/channel',
		...
		$scope.onUpdateRow = function()
		{
			...
			if( id >= 0 )	// Update
			{
				$http({
					method: 'PUT', 
					url: '/backoffice/callcenter/wizard/channel/' + id, 
			...
			else			
			{
				$http({
					method: 'POST', 
					url: '/backoffice/callcenter/wizard/channel', 
					data: $scope.model_data, 
					headers: {'Content-Type': 'application/json; charset=utf-8'} 
				})
				.success(function(data, status, headers, config) {
