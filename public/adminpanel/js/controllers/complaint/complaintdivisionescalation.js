define(['app', 'datatables.net', 'datatables.net-bs', 'directives/directive'], 
	...
	app.controller('ComplaintDivisionEscalationCtrl', function ($scope, $compile, $timeout, $http, $window,FileUploader /*$location, $http, initScript */) {
		console.log("ComplaintDivisionEscalationCtrl reporting for duty.");
		...
		$scope.fields = ['ID', 'Division', 'Severity', 'Levels', 'Job Roles', 'Max times', 'Notify Types'];
		function initDomData() {
			var $grid = $('#table_grid').dataTable( {
				processing: true,
				serverSide: true,
				order: [[ 0, "asc" ]], //column indexes is zero based
				ajax: '/backoffice/guestservice/wizard/complaintdivisionescalation',
				...
				"createdRow": function( row, data, dataIndex ) {
					...
					if( g_division_selected_id > 0 )	// already selected
					{
						if(  data.id == g_division_selected_id && data.severity_id == g_severity_selected_id )
						{
							$(row).attr('class', 'selected');	
							showLevelList();
							g_division_selected_id = 0;				
					...
					else 
					{
						...
						{					
							$(row).attr('class', 'selected');	
							$scope.division_selected_id = data.id;		
			...
			$grid.on( 'click', 'tr', function () {
				...
				$scope.division_selected_id = aData.id;
		...
		{	
			var data = {division_id: $scope.division_selected_id, severity_id: $scope.severity_selected_id};			
			
			$http({
				method: 'POST', 
				url: "/backoffice/guestservice/wizard/complaintdivisionescalation/selectitem",
				data: data, 
		...
		function addNewLevel() 
		{
			...
			row.id = 0;
			row.job_role_list = [];
			row.division_id = $scope.division_selected_id;
		...
		$scope.onDeleteLevel = function(row) {
			$http({
				method: 'POST', 
				url: "/backoffice/guestservice/wizard/complaintdivisionescalation/deleteinfo",
				data: row, 
				headers: {'Content-Type': 'application/json; charset=utf-8',
		...
		function refreshCurrentRow() {
			g_division_selected_id = $scope.division_selected_id;
		...
		function updateEscalationInfo(row) {
			...
			$http({
				method: 'POST', 
				url: "/backoffice/guestservice/wizard/complaintdivisionescalation/updateinfo",
				data: row, 
				headers: {'Content-Type': 'application/json; charset=utf-8',
						'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')} 
			})
