define(['app', 'datatables.net', 'datatables.net-bs', 'directives/directive'], 
	...
	app.controller('SubcomplaintLocEscalationCtrl', function ($scope, $compile, $timeout, $http, $window,FileUploader /*$location, $http, initScript */) {
		console.log("SubcomplaintLocEscalationCtrl reporting for duty.");
		
		//end///
		$scope.menus = [
					{link: '/complaint', name: 'Complaint'},
					{link: '/complaint/subcomplaintlocescalation', name: 'Sub Complaint Loc Escalation'},
				];
		...
		$scope.fields = ['ID', 'Locationi Type', 'Severity', 'Levels', 'Job Roles', 'Max times', 'Notify Types'];
		function initDomData() {
			var $grid = $('#table_grid').dataTable( {
				processing: true,
				serverSide: true,
				order: [[ 0, "asc" ]], //column indexes is zero based
				ajax: '/backoffice/guestservice/wizard/subcomplaintlocescalation',
				"lengthMenu": [[15, 25, 50, 100, -1], [15, 25, 50, 100, "All"]],
				columns: [
					//{ data: 'checkbox', width: '40px', orderable: false, searchable: false},
					{ data: 'id', name: 'slt.id' },
					{ data: 'type', name: 'slt.type' },
				...
				"createdRow": function( row, data, dataIndex ) {
					...
					if( g_loctype_selected_id > 0 )	// already selected
					{
						if(  data.id == g_loctype_selected_id && data.severity_id == g_severity_selected_id )
						{
							$(row).attr('class', 'selected');	
							showLevelList();
							g_loctype_selected_id = 0;				
					...
					else 
					{
						...
						{					
							$(row).attr('class', 'selected');	
							$scope.loctype_selected_id = data.id;		
			...
			$grid.on( 'click', 'tr', function () {
				...
				$scope.loctype_selected_id = aData.id;
		...
		{	
			var data = {type_id: $scope.loctype_selected_id, severity_id: $scope.severity_selected_id};			
			
			$http({
				method: 'POST', 
				url: "/backoffice/guestservice/wizard/subcomplaintlocescalation/selectitem",
				data: data, 
		...
		function addNewLevel() 
		{
			...
			row.id = 0;
			row.job_role_list = [];
			row.type_id = $scope.loctype_selected_id;
		...
		$scope.onDeleteLevel = function(row) {
			$http({
				method: 'POST', 
				url: "/backoffice/guestservice/wizard/subcomplaintlocescalation/deleteinfo",
				data: row, 
				headers: {'Content-Type': 'application/json; charset=utf-8',
		...
		function refreshCurrentRow() {
			g_loctype_selected_id = $scope.loctype_selected_id;
		...
		function updateEscalationInfo(row) {
			...
			$http({
				method: 'POST', 
				url: "/backoffice/guestservice/wizard/subcomplaintlocescalation/updateinfo",
				data: row, 
				headers: {'Content-Type': 'application/json; charset=utf-8',
						'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')} 
			})
