define(['app', 'datatables.net', 'datatables.net-bs', 'directives/directive'], 
		function (app) {
	app.controller('AdminareaCtrl', function ($scope, $compile, $timeout,$window, $http, FileUploader /*$location*/) {
		console.log("AdminareaCtrl reporting for duty.");
		
		$scope.model_data = {};
		$scope.menus = [
					{link: '/admin', name: 'Admin'},
					{link: '/admin/adminarea', name: 'Admin Area'},
				];
		...
		function initDomData() {
			var $grid = $('#table_grid').dataTable( {
				...
				ajax: '/backoffice/admin/wizard/admin',
		...
		$scope.onUpdateRow = function()
		{
			...
			if( id >= 0 )	// Update
			{
				$http({
					method: 'PUT', 
					url: '/backoffice/admin/wizard/admin/' + id, 
			...
			else			
			{
				$http({
					method: 'POST', 
					url: '/backoffice/admin/wizard/admin', 
					data: $scope.model_data, 
					headers: {'Content-Type': 'application/json; charset=utf-8'} 
				})
				.success(function(data, status, headers, config) {
		...
		$scope.onDownloadExcel = function() {
			...
			$window.location.href = '/backoffice/property/wizard/auditadminarea_excelreport?';
