define(['app', 'datatables.net', 'datatables.net-bs', 'directives/directive'],
    function (app) {
        ...
        app.controller('PartGroupCtrl', function ($scope, $compile, $timeout, $window, $http /*$location, $http, initScript */) {


            $scope.model_data = {};
            $scope.full_height = 'height: ' + ($window.innerHeight - 140) + 'px; overflow-y: auto;';
            $scope.menus = [
                {link: '/engineering', name: 'Part Group'},
                {link: '/engineering/partgroup', name: 'partgroup'},
            ];
            ...
            $scope.fields = ['ID', 'Name', 'Description','Code'];
            ...
            function initDomData() {
                ...
                var $grid = $('#table_grid').dataTable( {
                    ...
                    ajax: '/backoffice/engineering/wizard/partgroupnggrid/get',
                    "lengthMenu": [[15, 25, 50, 100, -1], [15, 25, 50, 100, "All"]],
                    columns: [
                        //{ data: 'checkbox', width: '40px', orderable: false, searchable: false},
                        { data: 'id', name: 'eg.id' },
                        { data: 'name', name: 'eg.name' },
                        { data: 'description', name: 'eg.description' },
                        { data: 'code', name: 'eg.code' },
            ...
            $scope.onUpdateRow = function()
            {
                ...
                if( id >= 0 )	// Update
                {
                    $http({
                        method: 'PUT',
                        url: '/backoffice/engineering/wizard/partgroup/' + id,
                ...
                else
                {
                    $http({
                        method: 'POST',
                        url: '/backoffice/engineering/wizard/partgroup',
                        data: $scope.model_data,
                        headers: {'Content-Type': 'application/json; charset=utf-8'}
                    })
                        .success(function(data, status, headers, config) {
            ...
            $scope.onDeleteRow = function(id)
            {
                if( id >= 0 )
                {
                    $http.get('/backoffice/engineering/wizard/partgroup/' + id)
