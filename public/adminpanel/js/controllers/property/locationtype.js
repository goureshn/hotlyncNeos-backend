define(['app', 'datatables.net',
	...
	app.controller('LocationTypeCtrl', function ($scope, $compile, $timeout, $http /*$location, $http, initScript */) {
				
		$scope.model_data = {};
		$scope.menus = [
					{link: '/property', name: 'Property'},
					{link: '/property/locationtype', name: 'Location Type'},
				];
		...
		//end///
		$scope.fields = ['ID', 'Type', 'Short Code'];
		function initDomData() {
			var $grid = $('#table_grid').dataTable( {
				processing: true,
				serverSide: true,
				order: [[ 0, "asc" ]], //column indexes is zero based
				ajax: '/backoffice/property/wizard/locationtype',
		...
		$scope.onUpdateRow = function()
		{
			...
			if( id >= 0 )	// Update
			{
				$http({
					method: 'PUT', 
					url: '/backoffice/property/wizard/locationtype/' + id, 
			...
			else			
			{
				$http({
					method: 'POST', 
					url: '/backoffice/property/wizard/locationtype', 
					data: $scope.model_data, 
					headers: {'Content-Type': 'application/json; charset=utf-8'} 
				})
				.success(function(data, status, headers, config) {
