	...
	<div class="row"> 
		<div class="col-sm-offset-1 col-md-10"> 
			...
			<div class="col-sm-offset-10">
				<button type="button" class="btn btn-warning btn-sm" onclick="location.href = '/backoffice/call/wizard/timeslab'"> 
	...
	<div class="modal-dialog" role="document"> 
		<div class="modal-content"> 
			<div class="modal-header"> 
				...
				<h3 class="modal-title" style="color:#2691d9" id="addModalLabel">Add Admin Rate Map</h3> 
			</div>                         
...
<script>
	...
	var $grid = $('#ratemap_grid').dataTable( {
		processing: true,
		serverSide: true,
		ajax: '/backoffice/call/wizard/adminrategrid/get',
	...
	{	
		...
		if( id > 0 )	// Update
		{
			$('#createButton').hide();
			$('#updateButton').show();
			$('#addModalLabel').text("Update Admin Rate Map");
			
			$.ajax({
				url: "/backoffice/call/wizard/adminrate/" + id,
				success:function(data){
					console.log(data);
					$('#carriergroup_id').val(data.carrier_groups);				
					$('#name').val(data.name);
					$('#timeslab_id').val(data.time_slab_group);
		...
		{
			$('#addModalLabel').text("Add Admin Rate Map");
			$('#createButton').show();
			$('#updateButton').hide();
		}		
	}
	...
	function onUpdateRow()
	{
		...
		var data = {
			id : id,
			name: $("#name").val(),
			carrier_groups: $('#carriergroup_id').val(),
			time_slab_group: $('#timeslab_id').val(),
		...
		if( id >= 0 )	// Update
		{
			$.ajax({
				type: "POST",
				url: "/backoffice/call/wizard/adminrate/updatedata",
				data: data,
				success:function(data){
		...
		{	
			...
			$.ajax({
				type: "POST",
				url: "/backoffice/call/wizard/adminrate/createdata",
				data: data,
				success:function(data){
					// $grid.fnDraw();
	...
	function  deleteRow()
	{
		...
		{
			$.ajax({
				url: "/backoffice/call/wizard/adminrate/delete/" + id,
				success:function(data){					
					refreshCurrentPage();					
				},			
				error:function(request,status,error){
