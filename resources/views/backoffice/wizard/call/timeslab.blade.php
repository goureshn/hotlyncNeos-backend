		...
		<div class="col-sm-offset-1 col-md-10"> 
			<div class="panel panel-primary"> 
				...
				<div class="panel-body"> 
					<table id="timeslab_grid" class="table table-hover table-bordered" style="text-align: center"> 
						<thead> 
							<tr> 
			...
			<div class="col-sm-offset-10">
					<button type="button" class="btn btn-warning btn-sm" onclick="location.href = '/backoffice/call/wizard/allowance'"> 
					<span class="glyphicon glyphicon-backward"></span><b> Back </b>
				</button>
					<button type="button" class="btn btn-success btn-sm" onclick="location.href = '/backoffice/call/wizard/adminrate'"><b> Next </b>  
		...
		<div class="modal-content"> 
			...
			<div class="modal-body"> 
				...
				<form class="form-horizontal"> 
					...
					<div class="form-group">
						<label class="control-label col-sm-4" for="client">Slab Name: </label>
						<div class="col-sm-5">
							<input type="text" class="form-control" id="name" placeholder="Name">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-4" for="client">Days: </label>
						<div class="col-sm-5">
							<?php echo Form::select('days_id[]', $timeslab->getDayList(), $timeslab->getDays(),  array('id' => 'days_list', 'class'=>'form-control', 'multiple')); ?>							
						</div>
					</div>	
					<div class="form-group">
						<label class="control-label col-sm-4" for="client">Start time: </label>
						<div class="col-sm-5">
							<input type="text" class="form-control" id="start_time" placeholder="Start time">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-4" for="client">End time: </label>
						<div class="col-sm-5">
...
<script>
	...
	var $grid = $('#timeslab_grid').dataTable( {
		processing: true,
		serverSide: true,
		ajax: '/backoffice/call/wizard/timeslabgrid/get',
	...
	{	
		...
		if( id > 0 )	// Update
		{
			$('#createButton').hide();
			$('#updateButton').show();
			$('#addModalLabel').text("Update Time Slab");
			...
			$.ajax({
				...
				success:function(data){
					...
					$.each(data.days_of_week.split(","), function(i,e){
						$("#days_list option[value='" + e + "']").prop("selected", true);
					});
		...
		{
			$('#addModalLabel').text("Add Time Slab");
			$('#createButton').show();
			$('#updateButton').hide();
		}		
	}
	...
	function onUpdateRow()
	{
		...
		var days = "";
		var count = 0;
		$("#days_list option:selected").each(function()
		{
			if( count == 0 )
				days = $(this).text();
			else
		...
		if( id >= 0 )	// Update
		{
			$.ajax({
				type: "POST",
				url: "/backoffice/call/wizard/timeslab/updatedata",
				data: data,
				success:function(data){
		...
		{	
			...
			$.ajax({
				type: "POST",
				url: "/backoffice/call/wizard/timeslab/createdata",
				data: data,
				success:function(data){
					// $grid.fnDraw();
	...
	function  deleteRow()
	{
		...
		{
			$.ajax({
				url: "/backoffice/call/wizard/timeslab/delete/" + id,
				success:function(data){					
					refreshCurrentPage();					
				},			
				error:function(request,status,error){
