		...
		<div class="col-sm-offset-1 col-md-10"> 
			<div class="panel panel-primary"> 
				...
				<div class="panel-body"> 
					<table id="carrier_charge_grid" class="table table-hover table-bordered" style="text-align: center"> 
						<thead> 
							<tr> 
								...
								<th style="text-align: center"> <b>Rate Description</b> </th> 
								<th style="text-align: center"> <b>Charge Method</b> </th> 
			...
			<div class="col-sm-offset-10">
				<button type="button" class="btn btn-warning btn-sm" onclick="location.href = '/backoffice/call/wizard/carriergroup'"> 
					<span class="glyphicon glyphicon-backward"></span><b> Back </b>
				</button>
					<button type="button" class="btn btn-success btn-sm" onclick="location.href = '/backoffice/call/wizard/propertycharge'"><b> Next </b>  
		...
		<div class="modal-content"> 
			...
			<div class="modal-body"> 
				...
				<form class="form-horizontal"> 
					...
					<div class="form-group"> 
						<label class="control-label col-sm-4" for="Property">Charge Method : </label>                                     
						<div class="col-sm-5"> 
							<?php echo Form::select('chargetype_id', $carrier_charge->getTypeList(), '1', array('class'=>'form-control', 'id' => 'chargetype_id')); ?>                                        
						</div>                                     
					</div>                                 
					<div class="form-group"> 
					...
					<div class="form-group"> 
						<label class="control-label col-sm-4" for="client">Rate : </label>                                     
						<div class="col-sm-5"> 
...
<script>
	...
	var $grid = $('#carrier_charge_grid').dataTable( {
		processing: true,
		serverSide: true,
		ajax: '/backoffice/call/wizard/carrierchargegrid/get',
	...
	{	
		...
		if( id > 0 )	// Update
		{
			$('#createButton').hide();
			$('#updateButton').show();
			$('#addModalLabel').text("Update Charge Group");
		...
		{
			$('#addModalLabel').text("Add Charge Group");
			$('#createButton').show();
			$('#updateButton').hide();
		}		
	}
	...
	function onUpdateRow()
	{
		...
		if( id >= 0 )	// Update
		{
			$.ajax({
				type: "POST",
				url: "/backoffice/call/wizard/carriercharge/updatedata",
				data: data,
				success:function(data){
		...
		{	
			...
			$.ajax({
				type: "POST",
				url: "/backoffice/call/wizard/carriercharge/createdata",
				data: data,
				success:function(data){
					// $grid.fnDraw();
	...
	function onDeleteRow(id)
	{
		...
		if( id >= 0 )
		{
			$.ajax({
				url: "/backoffice/call/wizard/carriercharge/" + id,
				success:function(data){
					$("#delete_item").text("\"" + data.description + "\"");		
				},			
				error:function(request,status,error){
					//alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
				}
	...
	function  deleteRow()
	{
		...
		{
			$.ajax({
				url: "/backoffice/call/wizard/carriercharge/delete/" + id,
				success:function(data){					
					refreshCurrentPage();					
				},			
				error:function(request,status,error){
