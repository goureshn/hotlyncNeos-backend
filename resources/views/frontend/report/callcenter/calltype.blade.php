...
<div>
        <table class="grid"  style="width : 100%;">
            ...
            <tr>
                <th><b>Date</b></th>
                @foreach ($data['call_type_names'] as $row)
                    <th><b>{{$row}}</b></th>
              @endforeach
            </tr>
            </thead>
            ...
            @for($j=0;$j<count($data['call_type_each']); $j++)
                <tr class="">
                    <td class="center">{{$data['summary'][$j]->date}}</td>
                    @foreach ($data['call_type_each'][$j] as $row )
            ...
            @endfor
            <tr>
                    <td colspan={{count($data['call_type_total'])+1}}> &nbsp;</td>
                </tr>
             <tr class="">
                    <td>{{$data['total']}}</td>
                    @foreach ($data['call_type_total'] as $row1)
...
@if( $data['report_type'] == 'Detailed')
    <div>
       ...
       @foreach ($data['detailed'] as $key => $row)
            <p style="margin-top: 5px; margin-bottom: 0px;" ><b>Call Type : {{$row->type}}</b></p>
